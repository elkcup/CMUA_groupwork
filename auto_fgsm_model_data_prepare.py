...
def init_attack_data(args_attack, attgan_args):
    ...
    test_dataloader = data.DataLoader(
        test_dataset, batch_size=8, num_workers=0,
...
def prepare(init_att=False,init_attention=False,init_star=False,init_HISD=False,init_all=False):
    ...
    if init_att:
        ...
        return attack_dataloader, test_dataloader, attgan, attgan_args
    if init_attention:
        ...
        print("Finished deepfake models initialization!")
        return attack_dataloader, test_dataloader, attentiongan_solver
    if init_star:
        solver = init_stargan(args_attack, test_dataloader)
        solver.restore_model(solver.test_iters)
        print("Finished deepfake models initialization!")
        return attack_dataloader, test_dataloader, solver
    if init_HISD:
        transform, F, T, G, E, reference, gen_models = prepare_HiSD()
        print("Finished deepfake models initialization!")
        return attack_dataloader, test_dataloader, transform, F, T, G, E, reference, gen_models
    if init_all:
        solver = init_stargan(args_attack, test_dataloader)
        solver.restore_model(solver.test_iters)
        attentiongan_solver = init_attentiongan(args_attack, test_dataloader)
