...
# init attack data
def init_attack_data(args_attack, attgan_args,batch_size=None):
    ...
    if args_attack.global_settings.num_test is None:
        print('Testing img_align_celeba:', len(test_dataset))
    else:
        print('Testing img_align_celeba:', min(len(test_dataset), args_attack.global_settings.num_test))
    return test_dataloader

# init inference data
def init_inference_data(args_attack, attgan_args, batch_size=None):
    test_dataset = CelebA(args_attack.global_settings.data_path, args_attack.global_settings.attr_path, args_attack.global_settings.img_size, 'test', attgan_args.attrs,args_attack.stargan.selected_attrs)
    test_dataloader = data.DataLoader(
        test_dataset, batch_size=batch_size if batch_size is not None else 1, num_workers=2,
        shuffle=False, drop_last=False
    )
    if args_attack.global_settings.num_test is None:
        print('Testing img_align_celeba:', len(test_dataset))
...
def prepare(batch_size=None):
    # prepare deepfake models
    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    args_attack = parse()

    attgan, attgan_args = init_attGAN(args_attack)
    attack_dataloader = init_attack_data(args_attack, attgan_args, batch_size=batch_size)
    test_dataloader = init_inference_data(args_attack, attgan_args, batch_size=batch_size)
    ...
    transform, F, T, G, E, reference, gen_models = prepare_HiSD(device)
