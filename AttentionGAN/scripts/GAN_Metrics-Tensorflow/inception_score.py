...
def get_inception_score(batch_size, images, inception_images, logits, splits=10):
    ...
    print('Calculating Inception Score with %i img_align_celeba in %i splits' % (images.shape[0], splits))
    start_time=time.time()
    preds = get_inception_probs(batch_size, images, inception_images, logits)
    mean, std = preds2score(preds, splits)
    print('Inception Score calculation time: %f s' % (time.time() - start_time))
    return mean, std  # Reference values: 11.34 for 49984 CIFAR-10 training set img_align_celeba, or mean=11.31, std=0.08 if in 10 splits.
