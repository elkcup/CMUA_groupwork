...
class AttentionGANModel(BaseModel):
    ...
    def __init__(self, opt):
        ...
        # specify the img_align_celeba you want to save/display. The training/test scripts will call <BaseModel.get_current_visuals>
        ...
        if self.isTrain:
            if opt.lambda_identity > 0.0:  # only works when input and output img_align_celeba have the same number of channels
                assert(opt.input_nc == opt.output_nc)
            self.fake_A_pool = ImagePool(opt.pool_size)  # create image buffer to store previously generated img_align_celeba
            self.fake_B_pool = ImagePool(opt.pool_size)  # create image buffer to store previously generated img_align_celeba
    ...
    def backward_D_basic(self, netD, real, fake):
        ...
        Parameters:
            ...
            real (tensor array) -- real img_align_celeba
            fake (tensor array) -- img_align_celeba generated by a generator
    ...
    def optimize_parameters(self):
        ...
        # forward
        self.forward()      # compute fake img_align_celeba and reconstruction img_align_celeba.
        # G_A and G_B
